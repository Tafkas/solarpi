...
def daily(date=datetime.now().strftime("%Y-%m-%d")):
    ...
    w = (
        Weather.query.with_entities(Weather.created_at, Weather.temp)
        .filter(Weather.created_at > current_date.strftime("%Y-%m-%d"))
        .filter(Weather.created_at < tomorrow.strftime("%Y-%m-%d"))
        .all()
    )
